...
// Das eigentliche Formular in eine eigene Komponente auslagern
function LoginForm() {
    ...
    <div className="w-full max-w-md space-y-8 bg-white p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 relative overflow-hidden">
      ...
      {showSuccess && !error && (
        <div className="absolute top-4 right-4 left-4 bg-emerald-50 border border-emerald-100 text-emerald-600 px-4 py-3 rounded-2xl flex items-center gap-2 animate-in fade-in slide-in-from-top-2">
          ...
          <span className="text-xs font-bold">Konto bereit! Logge dich jetzt ein.</span>
        </div>
      )}
      ...
      <div className="text-center pt-4">
        ...
        <h1 className="text-3xl font-black text-slate-900 tracking-tight">Willkommen zurück</h1>
        <p className="mt-2 text-slate-500 font-medium">Logge dich ein für PlannerPro</p>
      </div>
      ...
      <p className="text-center text-sm font-bold text-slate-400">
        Noch kein Konto? <Link href="/register" className="text-blue-600 hover:text-blue-700 ml-1">JETZT REGISTRIEREN</Link>
      </p>
    </div>
  );
}

// Der Export hüllt alles in Suspense ein
